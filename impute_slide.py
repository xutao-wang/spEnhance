...
def get_data(prefix):
    ...
    cnts = load_csv(f'{prefix}cnts.csv')
...
def predict(h, w, img_feature, model, names_list, prefix, y_range, patch_size=7, stride=1, device='cuda'):
    ...
    for i, gene in enumerate(names_list):
        ...
        save_pickle(gene_array, f'{prefix}/Primary_prediction/cnts-super/{gene}.pickle')
        print(f'{gene}.pickle saved to {prefix}/Primary_prediction/cnts-super/{gene}.pickle')
...
def impute(
    ...
    embs_1 = pad_sliding(embs, kernel_size=7, stride=5)
