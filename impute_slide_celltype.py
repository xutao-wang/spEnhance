...
def get_locs(prefix, target_shape=None):
    ...
    locs = load_csv(f'{prefix}locs_celltype.csv')
    ...
    if target_shape is not None:
        ...
        locs /= rescale_factor
    locs = np.nan_to_num(locs, nan=0, posinf=0, neginf=0)
...
def get_data(prefix):
    gene_names = read_lines(f'{prefix}cell-type-names.txt')

    cnts = load_csv(f'{prefix}proportion_celltype.csv')
    cnts = cnts.iloc[:, cnts.var().to_numpy().argsort()[::-1]]
    cnts = cnts[gene_names]

    embs = load_pickle(f'{prefix}embeddings-hist-merged.pickle')
    embs = np.concatenate([embs['vit'], embs['uni']]).transpose(1,2,0)
...
def predict(h, w, img_feature, model, names_list, prefix, y_range, patch_size=7, stride=1, device='cuda'):
    ...
    for i, gene in enumerate(names_list):
        ...
        save_pickle(gene_array, f'{prefix}/Cell_proportion/cnts-super/{gene}.pickle')
        print(f'{gene}.pickle saved to {prefix}/Cell_proportion/cnts-super/{gene}.pickle')
