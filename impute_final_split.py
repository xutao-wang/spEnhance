...
def get_data_split(prefix, cnts_train_name, cnts_val_name, locs_train_name, locs_val_name):
    with open(f'{prefix}gene-names-groups.txt', "r") as f:
        loaded_list = json.load(f)
    ...
    embs_train = load_pickle(f'{prefix}embeddings-combined-train.pickle')
    embs_train = np.concatenate([embs_train['uni'], embs_train['vit'], embs_train['combined']]).transpose(1,2,0)

    embs_val = load_pickle(f'{prefix}embeddings-combined-val.pickle')
    embs_val = np.concatenate([embs_val['uni'], embs_val['vit'], embs_val['combined']]).transpose(1,2,0)

    locs_train = get_locs_custom_random_split(prefix, "he.jpg", locs_train_name, target_shape=embs_train.shape[:2])
    locs_val = get_locs_custom_random_split(prefix, "he.jpg", locs_val_name, target_shape=embs_val.shape[:2])

    return embs_train, embs_val, cnts_train, cnts_val, locs_train, locs_val, cluster_size

def get_locs_custom_random_split(prefix, img_name, locs_name, target_shape=None):

    locs = load_csv(f'{prefix}/{locs_name}')
    ...
    if target_shape is not None:
        wsi = load_image(f'{prefix}/{img_name}')
...
def get_data_test(prefix):
    ...
    cnts = preprocess_cnts(load_csv(f'{prefix}cnts.csv'), gene_names)
    

    embs = load_pickle(f'{prefix}embeddings-combined-all.pickle')
    # embs = np.concatenate([embs['pos'], embs['his'], embs['rgb']]).transpose(1, 2, 0)
    embs = np.concatenate([embs['uni'], embs['vit'], embs['combined']]).transpose(1,2,0)
    
    locs = get_locs(prefix, target_shape=embs.shape[:2])
    return embs, cnts, locs, cluster_size
...
def get_model(
    ...
    locs_train, locs_val,
    radius, ori_radius, cluster_size, prefix, batch_size, epochs, lr,
...
def normalize(cnts_train, cnts_val):
...
def impute(
        ...
        cnts_train, cnts_val,
        locs_train, locs_val,
        radius, cluster_size, ori_radius, epochs, batch_size, prefix,
    ...
    (cnts_train, (cnts_train_min, cnts_train_max),
     cnts_val, (_, _)) = normalize(cnts_train, cnts_val)
    ...
    kwargs_list = [
        dict(
            x_train=embs_train, y_train=cnts_train, x_val=embs_val, y_val = cnts_val,
            locs_train=locs_train, locs_val=locs_val,
            radius=radius, ori_radius=ori_radius, cluster_size = cluster_size,
    ...
    cnts = preprocess_cnts(load_csv(f'{prefix}cnts.csv'), names)
    
    cnts = cnts.to_numpy().astype(np.float32)
    _, cnts_min, cnts_max = normalize_cnts(cnts)
    ...
    del embs
    predict(h, w, img_feature = embs_1, model_list=model_list, names_list=names, prefix=prefix, y_range = cnts_range,
...
def get_args():
    ...
    parser.add_argument('--locs_train_name', type=str)
...
def main():
    ...
    embs_train, embs_val, \
        cnts_train, cnts_val, \
        locs_train, locs_val, cluster_size = get_data_split(prefix, cnts_train_name, cnts_val_name,
                                                                 locs_train_name, locs_val_name)
    ...
    impute(
            ...
            cnts_train=cnts_train, cnts_val=cnts_val, 
            locs_train=locs_train, locs_val = locs_val,
            radius=radius, cluster_size = cluster_size,
